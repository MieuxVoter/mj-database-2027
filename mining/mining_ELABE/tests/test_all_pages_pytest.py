...
@pytest.mark.parametrize("page_num,expected_candidates", [
...
def test_page_extraction(page_num, expected_candidates):
    ...
    assert len(results) == expected_candidates, \
    ...
    for name, scores in results.items():
        total = sum(scores)
        assert total == 100, \
            f"Page {page_num}, {name}: total = {total}% (attendu 100%)"
    
    # Vérifier que chaque candidat a 5 scores
    for name, scores in results.items():
        assert len(scores) == 5, \
            f"Page {page_num}, {name}: {len(scores)} scores (attendu 5)"
...
def test_page17_specific_candidates():
    ...
    for name, expected_scores in expected.items():
        ...
        assert results[name] == expected_scores, \
            f"{name}: attendu {expected_scores}, obtenu {results[name]}"
